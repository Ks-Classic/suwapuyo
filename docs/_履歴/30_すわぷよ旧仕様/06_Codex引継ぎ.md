...
### まず読む（この順番・実装前に必ず）
1. `docs/_履歴/30_すわぷよ旧仕様/04_旧デモ詳細設計.md` ← **実装の正（全章）**。特に §3(召喚)・§4(体操)・§3.9(キャラ選択)・§6(共通基盤)・**§14(前方互換シーム＝最重要)**。
2. `docs/_履歴/30_すわぷよ旧仕様/05_絵コンテ・ナレーター台本.md` ← 演出の尺・イージング・SE・**村長/もぐぴよの実セリフ**。
3. `docs/_履歴/30_すわぷよ旧仕様/02_旧タスク一覧.md` の「🔴 最優先: すわぷよ感動デモ」セクション ← タスク **DEMO-001〜502**。これを順に消化する。
...
5. 参考規約: `docs/10_ふわふわランド/90_参考資料/06_旧実装バイブル.md`。
...
- **1オリジンpath配信前提**: `App.tsx` の `hostname.includes("ふわふわランド旧統合版")` 経路に乗らない。path/hashで判定。これで buddyStore(IndexedDB) が共有され同一端末召喚が動く。
