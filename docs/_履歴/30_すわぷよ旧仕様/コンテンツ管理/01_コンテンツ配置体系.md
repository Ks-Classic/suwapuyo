...
- `originals/` は原本、`display/` はアプリ表示用の派生、`下書き/` は未確定の検討素材とする。
- アプリコードからは `/content/...` の絶対パスを参照する。
- `public/素材/` と `public/videos/` には新規素材を追加しない。
...
4. 検討中の画像・スクリーンショットは `docs/30_すわぷよ旧仕様/コンテンツ管理/整理前素材/` に置き、採用時に `public/content/` へ昇格する。
