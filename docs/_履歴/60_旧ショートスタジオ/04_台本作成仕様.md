...
> 実証パターンの根拠: `05_拡散性調査.md`（あれば）／本ファイルの設計はそれに準拠。
...
- **②医療広告NG lint**: `治る/予防/効果/効能/改善/保証…` を台本テキストで自動検出し warning 表示（出典 `docs/20_事業/医療広告・コンテンツ方針.md`）。子ども×健康×SNSの一発アウトを機械で止める。
...
**分析の進め方**: 最初の30〜50本は探索（外れ値の定性レビュー＋中央値）。多変数の本格分析は100本超＆1変数あたり10本以上たまってから（`05_拡散性調査.md` 準拠）。早すぎる多変数分析はやらない。
