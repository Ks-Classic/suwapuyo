...
> 最終更新: 2026-06-25
> 位置づけ: `07_スポンサータップ表示仕様.md` の次段。スポンサー特例ではなく、**表示キャラ全員にタップ時コンテンツを紐づけるCMS化**の実装正本。
...
| コンテンツ関係 | 1キャラ最大1公開コンテンツ | Phase 1では `display_characters.tap_コンテンツ_id -> tap_コンテンツs.id` の1対1。履歴/複数下書きは後続 |
...
| tap_events meta | allowlist | `index`, `sourceType`, `コンテンツItemKind` など限定キーのみ保存 |
...
| source_image_path | text | No | 原本画像。サンプルは `public/content/ふわふわランド旧統合版/characters/originals/...` |
...
| tap_コンテンツ_id | uuid | No | 紐づく `tap_コンテンツs.id` |
...
- `tap_コンテンツ_id` は `tap_コンテンツs.id` を参照し、参照先削除時は `set null`。
...
- 医療/健康表現は `docs/20_事業/医療広告・コンテンツ方針.md` に従う。
...
|---|---|---:|---|
| id | uuid | Yes | 主キー |
| tap_コンテンツ_id | uuid | Yes | `tap_コンテンツs.id` |
...
| `character-コンテンツ` | タップコンテンツ画像/動画/音声 | true | 初期は画像5MB、動画50MB、音声20MB目安 |
...
DBに保存する `image_path` / `video_path` / `audio_path` / `thumbnail_path` は bucket名を含まない object path とする。public URLはRepositoryで `storage.from("character-コンテンツ").getPublicUrl(path)` から生成する。
...
- ローカルファイルパスを直接保存しない。選択ファイルを `character-コンテンツ` Storageへアップロードし、DBにはobject pathだけを保存する。
...
3. `display_characters.tap_enabled` と `tap_コンテンツs.is_published` を確認。
...
1. migration追加: enum, `display_characters`, `tap_コンテンツs`, `tap_コンテンツ_items`, `tap_events`, `character-コンテンツ` bucket。
2. `public/content/ふわふわランド旧統合版/characters/originals` のサンプル22体を `display_characters` にseed/upsertするSQLを追加。
3. Supabase型生成。
4. Repository層追加。

実装メモ:
- migration名は `20260625xxxx_create_character_コンテンツ_cms.sql`。
- enumは `character_source_type`, `display_character_status`, `tap_event_type`。
- `character-コンテンツ` bucketは `supabase/config.toml` ではなく既存方針に合わせてmigration SQLで作られている場合、既存 `202606230002_create_artworks_storage_bucket.sql` と同じ方式に揃える。
- RLSは `display_characters` / `tap_コンテンツs` / `tap_コンテンツ_items` は anon read/insert/update を許可する。`tap_events` は anon insert のみ許可する。
- Realtime publicationには `display_characters`, `tap_コンテンツs`, `tap_コンテンツ_items` を追加する。`tap_events` はRealtime対象外。
- サンプルseedは `image_path=/content/ふわふわランド旧統合版/characters/display/*.png`、`source_image_path=/content/ふわふわランド旧統合版/characters/originals/*.png` で固定する。
...
今回実装済み:
- migration: `display_characters`, `tap_コンテンツs`, `tap_コンテンツ_items`, `tap_events`, public `character-コンテンツ` bucket。
- seed: `public/content/ふわふわランド旧統合版/characters/display/*.png` のサンプル22体と、`sample-suusuu` の動画コンテンツ。
...
- `npx supabase db push` で `202606250001_create_character_コンテンツ_cms.sql` 適用済み。
...
- remote `sample-suusuu` は `tap_enabled=true`、公開済み `tap_コンテンツs` と `/content/02_ユアタイム/04_映像・開始終了カード/01_出展ブース紹介.mp4` を持つ。
- remote `character-コンテンツ` bucket はpublic、50MB、画像/動画/音声MIME許可。
...
- Vercel productionへ反映済み。alias: `https://ふわふわランド旧統合版.vercel.app`、latest deployment: `https://ふわふわランド旧統合版-6aanq8j27-ks-classic.vercel.app`。
...
CMS実装後:
- `sponsors[]` は `display_characters` + `tap_コンテンツs` に移行。
- `SponsorPopup` は `CharacterContentPopup` に置き換え。
- すーすー動画は `sample-suusuu` の `tap_コンテンツs` としてseedする。

## 10. 実装前承認事項

仕様上の未決定はない。実装前にユーザー承認が必要な操作だけを明示する。

1. DBスキーマ変更: `display_characters`, `tap_コンテンツs`, `tap_コンテンツ_items`, `tap_events` の追加。
2. Storage bucket追加: public `character-コンテンツ`。
3. Supabase型再生成。
...
- `character_id`, `tap_コンテンツ_id`, `item_id`, `event_type`, `created_at` を集計キーとして維持する。
- 出展社概念を追加する時は `exhibitors` / `sponsor_slots` / `campaigns` / `report_スナップショット` を別テーブルで足し、既存tapログを壊さない。
