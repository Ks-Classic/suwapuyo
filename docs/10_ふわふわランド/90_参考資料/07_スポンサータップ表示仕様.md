...
> 最終更新: 2026-06-25
> 位置づけ: 槍B「もぐぴよtap→popup→送客＋計測」の**第1スライス**。本書は実装の正。`06_旧実装バイブル.md` の規約に従う。
...
| 設定 | `src/fuwafuwa-land/config.ts` / `src/fuwafuwa-land/renderer/sampleCharacters.ts` | `sponsors` と、`public/content/ふわふわランド旧統合版/characters/originals` 由来のデフォルトサンプルキャラを管理 |
...
export interface SponsorSlide {
  src: string;          // 例 "/content/02_ユアタイム/04_映像・開始終了カード/01_出展ブース紹介.mp4" or "/content/shorts-studio/下書き/yourtime-popup-step-1.png"。読み込み失敗時はスキップ
  kind?: "image" | "video";
  alt?: string;
}
...
- 正本: `public/content/ふわふわランド旧統合版/characters/originals/`。ここにあるキャラ画像をデフォルトサンプルキャラの正とする。
- 表示用派生: `public/content/ふわふわランド旧統合版/characters/display/`。原本を最大1024pxに縮小し、ASCIIファイル名で配置する。PixiJS/WebGLのテクスチャメモリを抑えるため、アプリ表示は原則こちらを参照する。
...
5. スタッフページの表示キャラ管理に `public/content/ふわふわランド旧統合版/characters/originals` 由来のサンプル全員が出る。表示/非表示/削除ボタンは小さく、一覧性を優先する。
...
| 編集 | `renderer/sampleCharacters.ts` | `public/content/ふわふわランド旧統合版/characters/originals` 由来のデフォルトサンプル全員・仮想Artwork生成 |
...
| 追加アセット | `public/content/ふわふわランド旧統合版/characters/display/*.png`（原本から最大1024pxへ縮小した表示用派生）／`public/content/02_ユアタイム/04_映像・開始終了カード/01_出展ブース紹介.mp4` |
