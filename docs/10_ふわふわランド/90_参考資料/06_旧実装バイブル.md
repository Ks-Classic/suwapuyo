...
> 最終更新: 2026-06-23 / **これは実装の正**。`05_旧サマリー/04_意思決定・ゲート計画.md`(決定)と本ファイル(実装粒度)を読めば、迷わず着手できる状態を目指す。
...
> 🎨 **素材適用（2026-06-23追記）**: ふわふわランド表示画面は既存ゲームの `public/content/01_すわぷよ/03_背景/01_村_昼.png` と `public/content/ふわふわランド旧統合版/sprites/{ghost,tooth,blob,tanuki}/idle.png` をそのまま使う。作品0件時は4キャラが待機表示として漂い、作品登録後は登録作品を優先表示する。
...
```tsx
// 既存 import に追加
import { FuwafuwaApp } from "./ふわふわランド旧統合版";

function App() {
  const isFuwafuwa = typeof window !== "undefined" && (
    window.location.hostname.includes("ふわふわランド旧統合版") ||
...
- サンプル: `config.sampleCharacters` / `sampleCharacters.ts` は既存 `public/content/ふわふわランド旧統合版/sprites/*/idle.png` を参照する。作品登録後も削除せず、世界観の住人として背面寄りに漂わせる。
...
- `03_旧詳細設計/02_API仕様.md` の `processArtworkImage(removeWhiteBackground)` は、本書の**マーカー切り出し＋連結背景透過前提**に更新（白背景除去単独ではなく「マーカー展開＋edge flood fill」）。api-spec はインターフェース名のみ整合させ、実体は本書を正とする。
- `02_旧基本設計/02_テーブル定義.md` に対し、本書 `types.ts` は `givenName/source/imageBlobKey/width/height` を追加（IndexedDB Blob前提）。table-definitions は注記で追従。
- 旧統合版メモ(`要件定義.md`等)は参照しない（番号付き＋本書が正）。
- ID体系: タスクは `FL-G##`(Gate)/`FL-E##`(Event) に統一済（TODO.md）。
- 2台連結は不採用、**1台横長を正**（renderer は単一widescreen canvas前提）。

> 着手順は `05_旧サマリー/04` の Day1→Day3。Day1は FL-G01(ストア)→FL-G02/03(入力)。型(`types.ts`)と `config.ts` を最初に確定してから各層へ。
