...
> 最終更新: 2026-06-25  
> 位置づけ: `00_旧企画・戦略/01_関係者価値・事業モデル.md` の「Phase 1の送客ダッシュボードが課金の心臓」を、技術・運用・将来拡張へ落とす仕様。  
...
2. **送客リンクのUTM/QR**  
   CTA URLに `utm_source=fuwafuwa_land`, `utm_campaign=yourtime_2026`, `utm_コンテンツ={exhibitor_id}` を付ける。
...
| tap_コンテンツ_id | uuid | tap_コンテンツs |
| plan_name | text | free/demo/paid/sponsor |
| starts_at | timestamptz | |
| ends_at | timestamptz | |

### 4.4 external_channel_スナップショット
...
- `tap_コンテンツ_publish_logs`: いつ公開/非公開になったか。
- `daily_report_metrics`: 日別の集計スナップショット。
- `external_channel_スナップショット`: SNS/LINE/YouTubeの手入力/APIスナップショット。
