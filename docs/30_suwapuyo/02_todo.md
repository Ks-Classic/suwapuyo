...
- [ ] **PROCESS-105**: 最初の公開記事を1本作り、X / Instagram / note / 自社サイトへの媒体別派生を作成
...
- [ ] **BRAND-102**: AI実験キャラを公式やすの無断コピーにせず、別デザインまたは明示許諾された派生として確定
- [ ] **BRAND-103**: やすキャラとAI実験キャラの性格、口調、ぶれてよい範囲、禁止表現、危機対応をキャラクターバイブル化
- [ ] **BRAND-104**: すわぷよLINE / ツナやすX・Instagram / ツナやすWeb / AI実験シリーズの役割とアカウント名を確定
...
- [ ] **CHAR-302**: 手動選択枠を固定し、新規ゲームでは未固定枠だけ再抽選する（固定/解除トグルと`rerollUnpinnedSlots()`は実装・テスト済み。配線前に木幡さんが次から選択する: A=キャラ選択画面mount時、B=`この4枠で遊ぶ`クリック時〔推奨〕、C=ゲーム終了後の明示的な`新しいゲーム`操作時。合意前は配線しない）
