...
- [ ] **ONBOARD-306**: 子ども単位の確認・変更・削除、同意撤回、全削除UIを実装
- [ ] **ONBOARD-307**: 人数先行選択、全児一括保存、戻る、中断再開、保存失敗を実装（人数・全児入力・戻る・一括保存まで実装済み。中断再開・保存失敗が残り）
...
- [ ] **CHAR-301**: ゲーム4枠を初回自動編成し、全キャラの姿と名前を選択可能にする
- [ ] **CHAR-302**: 手動選択枠を固定し、新規ゲームでは未固定枠だけ再抽選する
- [ ] **CHAR-303**: `おまかせ編成`で全枠再抽選し、直近未登場キャラを少し優先する
- [ ] **CHAR-304**: 自動編成、選択、固定、再抽選、登場偏りを計測・テストする

### LINEリッチメニュー [LINE-2xx]

- [x] **LINE-201**: 開催前・当日・開催後の5枠と役割を決定
- [ ] **LINE-202**: 3フェーズ×5枠のラベル、座標、遷移先、計測、fallback対応表を固定
...
- [ ] **BOOTH-104**: 確認済み台帳を型付きデータへ変換し、未確認値を本番へ出さないvalidatorを実装
- [ ] **BOOTH-105**: LIFF一覧、詳細、カテゴリ、検索、SNS導線、素材なしfallbackを実装（デモ3件の一覧・詳細・全画像カルーセル・Instagram導線まで実装。検索・fallbackが残り）
...
- [ ] **CHECKIN-102**: 入口QRのデモ導線と当日限定キャラクターの冪等付与演出を実装
- [ ] **CHECKIN-103**: 仮ブース2〜3件のQR／デモボタン、スタンプ付与、スタンプ帳を実装
- [ ] **CHECKIN-104**: 説明・体験・購入・見学の複数選択、任意評価・短文、スキップを実装
- [ ] **CHECKIN-105**: 出展者向け仮レポートで訪問と自己申告を分け、指標定義を表示
...
- [ ] **MAKER-102**: `すわぷよの作り手`ページを、実例→役割→課題別入口→相談確認の順で実装
- [ ] **MAKER-103**: `もっと覚えてもらいたい / 発信や仕事をラクに / 新しい体験を`の課題別CTAを実装
- [ ] **MAKER-104**: 相談文を利用者が確認・編集してから送信し、本人の意図なしに自動送信しない
- [ ] **MAKER-105**: maker viewed→problem selected→consultation startedを匿名計測し、営業台帳と分離
