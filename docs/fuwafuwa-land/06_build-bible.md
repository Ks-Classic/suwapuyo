...
- 裏モード: サンプルの `sample-tooth`（わーわー）は `pointertap` 対象。タップごとに `/assets/audio/suwa-good-morning.mp3` を再生し、`tapWindowMs` 内に5回タップされたら `わーわーもーど!` の大きな回転演出を出す。裏モード中は表示中の全キャラの見た目とラベルをわーわーに差し替える。再度5回タップで元に戻す。登録作品のSupabase画像/メタデータは変更しない。
