...
> 対象は **2026-07-02 打合せ用の感動デモ**（`03_village-concierge-design.md` §12）。8/2本番フル機能は別フェーズ。
...
2. 既存コードのアンカー（読んでから触る）:
   ...
   - `public/content/fuwafuwa-land/backgrounds/`（`map_sample.jpg` をここ等に配置して使う）。
...
### 絶対に守る制約
- **1オリジン path 配信**: `App.tsx` の `hostname.includes("fuwafuwa-land")` 経路に乗らない。path/hash で判定。新ルート `/concierge`（来場者）/`/concierge/staff`（アナウンス送信）/QRは `?booth=`。
...
通しの骨格（友だち追加→家族アンケート→マップ→QR→スタンプ→スタンプ帳→会場アナウンス）は実装済み。2026-07-01 のセルフレビューで「★感動の目玉」が仕様（§12.3-B/E）より薄かった点を以下の通り埋めた。詳細は `03_village-concierge-design.md` §12.6 参照。
