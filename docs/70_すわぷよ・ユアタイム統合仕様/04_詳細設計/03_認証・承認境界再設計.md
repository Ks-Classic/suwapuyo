...
>
> 運営認証: 2026-07-19、B案（Cloudflare Access）を木幡さん承認。migration・本番設定は別途承認を得るまで実行しない。
...
| 領域 | 実装済み | 未実装・仮実装 | 判定 |
|---|---|---|---|
| おえかきkiosk | 作品queued＋表示キャラhidden登録、スタッフUI、Realtime表示、送信後導線、undo/resize、`/staff/*` deep link | 登録・承認のDB transaction、運営本人性、private画像配信 | ローカルUI修正済み。承認境界は未完成 |
...
| お口体操 | 60秒周期、10種、タイプ別15〜45秒、一時停止・スキップ・明示完了のUIとunit test | 秒数の専門家監修、実機音声/タイマー、利用者DB記録 | 体験デモ段階 |
...
3. DB transactionで`artworks(status=queued)`と対応する`display_characters(status=hidden)`を同時作成する。queuedは承認待ちworkflowであり、公開可否は表示キャラのstatusで判定する。
...
3. 作品がqueued、キャラがhiddenで、対応関係が一致することを検証する。
4. `artworks.status='visible'`と`display_characters.status='visible'`を同一transactionで更新する。
...
| B | Cloudflare Access + WorkerでAccess JWT検証 | `/staff*`と管理APIを入口から閉じやすく、現行Cloudflare構成と整合 | Access policy、許可mail、緊急解除手順の管理が必要 | **採用** |
...
- 既存のqueued作品がvisible表示キャラへbackfillされていないかを監査し、不整合を非公開側へ補正する。
...
1. 通常表示が上限に達した場合の退場方針。
2. kiosk端末資格の配布・失効・オフライン運用。
3. キャラを単独所有にするか家族共有可能にするか。
