...
6. すわぷよのお口体操v2: プレイ中30秒に1回、首なし・お口と顔系のみ10種、キャラ登場、ミッション形式（できた！必須・スキップ不可・自動消滅なし）、開始時の間とワクワク演出、体操ごとに異なる音楽
...
- プレイ中**30秒に1回**自動で発生（ゲームは一時停止）。
- **首の体操は廃止**。お口・顔系のみ。
- **スキップ不可・自動消滅なし**。「できた！」ボタンを押すまで閉じない（唯一の出口）。
- 開始時は「いい感じの間」→「いっくよ〜！」のワクワク演出→体操本編。
- 体操ごとに**すべて異なる音楽**（WebAudio合成、イントロジングル＋本編ループ）。
- ホストキャラは22体からローテーション登場（直近出た子は避ける重み付き抽選）。

### 6.2 体操10種（お口・顔のみ）

| id | 名前 | かけ声ステップ | ねらい | 音楽の性格 |
|---|---|---|---|---|
| `aan` | おおきくあーん | あ・あ・あーん | 開口 | 明るいマーチ |
| `niko_ii` | にっこりいー | い・い・いー | 口角挙上 | スキップ調 |
| `tako_uu` | たこさんうー | う・う・うー | 口唇突出 | コミカルなワルツ |
| `pukupuku` | ぷくぷくほっぺ | ぷく・ぷく・ぱっ | 頬筋 | ぽこぽこ打楽器 |
| `beee` | べーっとした | べ・べ・べー | 舌出し | おどけたブルース |
| `pachipachi` | まばたきぱちぱち | ぱち・ぱち・ぎゅっ・ぱっ | 眼輪筋 | 軽快なピチカート |
| `wink` | かためウィンク | みぎ・ひだり・みぎ・ひだり | 表情筋左右 | ジャズ風 |
| `aiube` | あいうべたいそう | あ・い・う・べー | 口腔機能総合 | 王道体操曲 |
| `fuusen` | ほっぺふうせん | みぎぷく・ひだりぷく・こうたい | 頬交互 | ラテン風 |
| `chu` | ちゅーのくち | ちゅ・ちゅ・にこっ | 口輪筋 | かわいいポップ |

### 6.3 演出シーケンス

1. **予告（1.2秒）**: 画面が少し暗転、「♪」アイコンとドラムロール、ホストキャラがぴょこんと登場。
2. **タメ（0.8秒）**: ホストが息を吸うアニメ＋無音の間。
3. **かけ声（1秒）**: 「いっくよ〜！」（体操ごとに文言バリエーション: いっくよ〜！/ じゅんびはいい？/ おくちのじかんだよ〜！等）＋イントロジングル。
4. **本編**: かけ声ステップをビート同期（既存BEAT_MS=1500踏襲）でピクトグラム＋大きなかな表示。本編ループ音楽。
5. **完了待ち**: ステップ終了後「できたら おして！」→大きな「できた！」ボタン。**押すまで閉じない**。押すと拍手ファンファーレ＋`incrementTaisouCount("mouth")`＋スタンプ演出→ゲーム再開（30秒タイマーリセット）。
...
- `DemoScreen`: 30秒インターバルタイマー（体操中・バトル演出中は停止）、`?taisou=1`初回起動は維持。
...
> 発注意図: 木幡さん2026-07-19指示（各キャラが暮らし感のあるひとことをふきだしで喋る。管理画面から追加/削除・頻度設定・誰に喋らせるかを制御できる。将来的にはどのキャラがどのブースを紹介するかにも繋げたいので、DBで持ってfuwafuwa-land表示とすわぷよ側ブース紹介の両方から参照できる形にする）
...
- 30秒タイマー・スキップ不可・できた！必須はvitestでコンポーネントテストを追加。
