...
- **イベントメニュー**: スタッフ画面から `display_state.display_event` を更新し、ディスプレイ画面がRealtimeで演出を開始する。MVPは `バトル` と `イベント停止`。音は既存 `suwa-good-morning.mp3` を使い、ブラウザ自動再生制限対策としてディスプレイに `音ON` を置く。
...
- [ ] **FL-G06f**: バトル専用SE/BGMを追加し、音量/ミュート/演出尺をスタッフ画面で調整できるようにする
